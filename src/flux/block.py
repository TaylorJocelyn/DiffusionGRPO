...
#     key = attn.to_k_hs(hidden_states)
#     value = attn.to_v_hs(hidden_states)
...
#         # linear proj
#         hidden_states = attn.to_out_hs[0](hidden_states)
#         # dropout
#         hidden_states = attn.to_out_hs[1](hidden_states)
#         encoder_hidden_states = attn.to_add_out(encoder_hidden_states)
...
#     norm_hidden_states, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.norm1_hs(
#         hidden_states, emb=temb
#     )
...
#     # 1. hidden_states
#     ff_output = self.ff_hs(norm_hidden_states)
#     ff_output = gate_mlp.unsqueeze(1) * ff_output
#     # 2. encoder_hidden_states
#     context_ff_output = self.ff_context(norm_encoder_hidden_states)
#     context_ff_output = c_gate_mlp.unsqueeze(1) * context_ff_output
...
#     norm_hidden_states, gate = self.norm_hs(hidden_states, emb=temb)
#     mlp_hidden_states = self.act_mlp(self.proj_mlp_hs(norm_hidden_states))
#     if using_cond:
#         residual_cond = condition_latents
#         norm_condition_latents, cond_gate = self.norm(condition_latents, emb=cond_temb)
#         mlp_cond_hidden_states = self.act_mlp(self.proj_mlp(norm_condition_latents))
...
#     hidden_states = torch.cat([attn_output, mlp_hidden_states], dim=2)
#     gate = gate.unsqueeze(1)
#     hidden_states = gate * self.proj_out_hs(hidden_states)
#     hidden_states = residual + hidden_states
#     if using_cond:
#         condition_latents = torch.cat([cond_attn_output, mlp_cond_hidden_states], dim=2)
#         cond_gate = cond_gate.unsqueeze(1)
