...
from rewards import load_reward_models
...
def load_flux_transformer(device):
...
def set_params_trainable(transformer):
    with open('/mnt/workspace/user/zengdawei.zdw/shared/jinghan.zq/DiffusionRL/debug/params.txt', 'w') as f:
    ...
    with open('/mnt/workspace/user/zengdawei.zdw/shared/jinghan.zq/DiffusionRL/debug/lora_params.txt', 'w') as f:
        for name, param in transformer.named_parameters():
            if 'lora' in name:
                f.write(name + '\n')
...
def run_sample_step(
    ...
    if grpo_sample:
        ...
        # prompts = batch["description"] # ['Marketing, promotion...he product']
        position_delta = position_delta[0]
        
        # Prepare inputs
        with torch.no_grad():
            vae_scale_factor_ = (
                2 ** (len(flux_pipe.vae.config.block_out_channels)) if hasattr(flux_pipe, "vae") and flux_pipe.vae is not None else 16
            )
            ...
            for cond_ in conditions:
                ...
                cond_latents, cond_ids = encode_cond_images(
                    ...
                    height=h,
                    width=w,
                    vae_scale_factor=vae_scale_factor_
                    ) # (1, 1024, 64) (1024, 3)
        ...
        for i in progress_bar:  # Add progress bar
            ...
            z, pred_original, log_prob = flux_step(pred, z.to(torch.float32), args.eta, sigmas=sigma_schedule, index=i, prev_sample=None, grpo=True, sde_solver=True) # log_prob = log[p(x_{t-1})]
            z.to(torch.bfloat16)
...
def train_one_step(
    ...
    for k, v in samples.items():
        if k in ('advantages', 'rewards', 'image_ids', 'condition_latents', 'condition_ids', 'condition_type_ids'):
...
def main(args):
    ...
    # load reward models
    ctr_model, hpsv2_model, hpsv2_preprocess_val, hpsv2_processor, ecp_model = load_reward_models(args, device, logger)

    main_print(f"--> loading model from {args.pretrained_model_name_or_path}", logger)
    # keep the master weight to float32
    # Load FLUX Transformer
    flux_pipe = load_flux_transformer(device)
    ...
    # with open('/mnt/workspace/user/zengdawei.zdw/shared/jinghan.zq/DiffusionRL/debug/untrainable_params.txt', 'w') as f:
    #     for name, param in transformer.named_parameters():
