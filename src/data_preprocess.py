...
def collect_prompts():
    data_path = "/root/autodl-tmp/DiffusionGRPO/data/outputs/mixgrpo/train.json"
    import json, random
    with open(data_path, 'r') as file:
        data = json.load(file)

    df = pd.DataFrame(data)
    prompts = list(set(df["prompt"].values.tolist()))

    random.shuffle(prompts)
    prompts = prompts[:20000]
    
    with open('/root/autodl-tmp/DiffusionGRPO/data/rl_embeddings_hpd/prompts.txt', 'w') as file:
