...
def prepare_sequence_parallel_data(
    encoder_hidden_states, pooled_prompt_embeds, text_ids, caption, title, pcate_lv3_name, ori_img, canny_img, depth_img, position_delta
):
    ...
    def prepare(
        ...
        ori_img = all_to_all(ori_img, scatter_dim=1, gather_dim=0)
        canny_img = all_to_all(canny_img, scatter_dim=1, gather_dim=0) 
        depth_img = all_to_all(depth_img, scatter_dim=1, gather_dim=0)
        position_delta = all_to_all(position_delta, scatter_dim=1, gather_dim=0)
        return (
            encoder_hidden_states,
            pooled_prompt_embeds,
            text_ids, 
...
def sp_parallel_dataloader_wrapper(
    ...
    while True:
        for data_item in dataloader:
            encoder_hidden_states, pooled_prompt_embeds, text_ids, caption, title, pcate_lv3_name, ori_img, canny_img, depth_img, position_delta = data_item
            #latents = latents.to(device)
            encoder_hidden_states = encoder_hidden_states.to(device)
            pooled_prompt_embeds = pooled_prompt_embeds.to(device)
            text_ids = text_ids.to(device)
            ori_img = ori_img.to(device) 
            canny_img = canny_img.to(device)
            depth_img = depth_img.to(device)
            position_delta = position_delta.to(device)

            #frame = latents.shape[2]
            frame = 19
            if frame == 1:
                yield encoder_hidden_states, pooled_prompt_embeds, text_ids, caption, title, pcate_lv3_name, ori_img, canny_img, depth_img, position_delta
            else:
                encoder_hidden_states, pooled_prompt_embeds, text_ids, caption, title, pcate_lv3_name, ori_img, canny_img, depth_img, position_delta = prepare_sequence_parallel_data(
                    encoder_hidden_states, pooled_prompt_embeds, text_ids, caption, title, pcate_lv3_name, ori_img, canny_img, depth_img, position_delta
                )
                assert (
                ...
                for iter in range(train_batch_size * sp_size // train_sp_batch_size):
                    ...
                    ori_img = ori_img[st_idx:ed_idx]
                    canny_img = canny_img[st_idx:ed_idx]
                    depth_img = depth_img[st_idx:ed_idx]
                    position_delta = position_delta[st_idx:ed_idx]
                    yield (
                            encoder_hidden_states,
                            pooled_prompt_embeds,
                            text_ids, 
