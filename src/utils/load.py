...
from src.models.mochi_hf.modeling_mochi import (MochiTransformer3DModel,
                                                      MochiTransformerBlock)
from src.utils.logging_ import main_print
...
def load_lora_weights(
        transformer, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], adapter_name=None, logger=None, **kwargs
    ):
        """
        ...
        has_norm_keys = any(
            norm_key in key for key in state_dict.keys() for norm_key in FluxLoraLoaderMixin._control_lora_supported_norm_keys
        )
        ...
        transformer_norm_state_dict = {
            ...
            if "transformer." in k and any(norm_key in k for norm_key in FluxLoraLoaderMixin._control_lora_supported_norm_keys)
        }

        has_param_with_expanded_shape = FluxLoraLoaderMixin._maybe_expand_transformer_param_shape_or_error_(
        ...
        transformer_lora_state_dict = FluxLoraLoaderMixin._maybe_expand_lora_state_dict(
        ...
        if len(transformer_norm_state_dict) > 0:
            transformer._transformer_norm_layers = FluxLoraLoaderMixin._load_norm_into_transformer(
                transformer_norm_state_dict,
                transformer=transformer,
