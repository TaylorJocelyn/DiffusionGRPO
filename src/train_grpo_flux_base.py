...
from utils.communications_flux import sp_parallel_dataloader_wrapper, sp_parallel_dataloader_wrapper_uncond
...
from latent_flux_rl_datasets import LatentDataset, UncondLatentDataset, latent_collate_function, uncond_latent_collate_function
import torch.distributed as dist
from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
from utils.checkpoint import (
    save_checkpoint,
    save_lora_checkpoint,
    save_base_and_lora_checkpoint
)
...
def train_one_step(
    ...
    rewards, all_latents, all_log_probs, sigma_schedule, all_image_ids = sample_reference_model(
            args,
            device, 
            vae,
            transformer,
    ...
    for i, sample in list(enumerate(samples_batched_list)):
        ...
        if dist.get_rank()%8==0:
            ...
            main_print(f"ratio: {ratio}", logger)
...
def main(args):
    ...
    train_dataset = UncondLatentDataset(args.data_json_path, args.num_latent_t, args.cfg, device)
    ...
    train_dataloader = DataLoader(
        train_dataset,
        sampler=sampler,
        collate_fn=uncond_latent_collate_function,
        pin_memory=True,
    ...
    if rank <= 0:
        project = "flux"
        wandb.init(project=project, config=args, name='flux-base-aesthetics')
    ...
    loader = sp_parallel_dataloader_wrapper_uncond(
        train_dataloader,
        device,
        args.train_batch_size,
        args.sp_size,
    ...
    for epoch in range(1):
        ...
        for step in range(init_steps+1, args.max_train_steps+1):
            ...
            # if step % 1 == 0:
                save_checkpoint(transformer, rank, args.output_dir, step, epoch, args.task_type, logger)
